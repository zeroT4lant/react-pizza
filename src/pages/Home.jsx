...
export const Home = () => {
  const dispatch = useDispatch();//для категорий
  ...
  useEffect(() => {
    ...
    axios.get(`https://1e1f1345ed33866a.mokky.dev/items?page=${currentPage}&limit=4&${category}&sortBy=${sortBy}${search}`)
    .then(res => {//ответ от сервера в res
...
//<Sort ssort={ssort}/> было - попробуй убрать ssort
